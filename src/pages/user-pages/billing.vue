<template>
  <div class="flex flex-col h-screen bg-gradient-to-br from-gray-50 to-gray-100 text-gray-800 overflow-hidden">
    ...
    <div class="flex flex-1 min-h-0 overflow-hidden">
      ...
      <!-- Main Content -->
      <main class="flex-1 px-3 sm:px-6 py-3 sm:py-6 space-y-3 sm:space-y-4 overflow-y-auto h-full scroll-smooth text-sm">
        <transition name="fade" mode="out-in">
          ...
          <!-- Main Content -->
          <div v-else key="content" class="space-y-3 sm:space-y-4 animate-fade-in flex flex-col">

            <!-- ========================= SEARCH & FILTER SECTION ========================= -->
            <section class="bg-white p-3 sm:p-4 rounded-xl border border-gray-200 shadow-sm space-y-2">
              <!-- Mobile Search Icon -->
              ...
              <transition name="slide-down">
                <div v-if="showMobileSearch" class="sm:hidden mb-1">
                  <input
                    ...
                    class="w-full px-2 py-1.5 border border-gray-300 rounded-lg focus:ring-1 focus:ring-blue-500 text-xs shadow-sm"
                  />
                </div>
              </transition>
              ...
              <div class="flex gap-1.5 overflow-x-auto no-scrollbar pt-1">
                <button
                  ...
                  :class="[
                    'flex-shrink-0 px-2 py-1 rounded-full text-[10px] font-semibold whitespace-nowrap transition',
            ...
            <section class="space-y-2">
              <div v-for="category in orderedCategories" :key="category" class="space-y-1">
                <div v-if="groupedFilteredServices[category]">
                  <h3 class="flex items-center gap-1.5 text-xs font-semibold text-green-600 mt-1 mb-1 py-1 border-b">
                    <span class="w-2 h-2 bg-green-500 rounded-full"></span>
                    <List class="w-3 h-3" /> {{ category }}
                  </h3>

                  <div class="grid gap-1.5 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
                    <label
                      v-for="service in groupedFilteredServices[category]"
                      :key="service.id"
                      class="flex items-center justify-between p-2 border border-gray-200 rounded-lg bg-white shadow-sm hover:shadow-md cursor-pointer transition"
                    >
            ...
            <div class="pt-2 flex justify-center">
              <button
                ...
                class="w-full sm:w-auto flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white text-sm font-semibold px-4 py-1.5 rounded-md shadow-md transition"
              >
                <svg
                  v-if="buttonLoading"
